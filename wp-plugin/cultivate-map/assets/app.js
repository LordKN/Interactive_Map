...
const pluginAssetsUrl = cultivateMapAjax.plugin_url + "assets/"; // e.g. "/wp-content/plugins/cultivate-map/assets/"

const chartsConfig = [
  { file: pluginAssetsUrl + "2023_log.csv", canvasId: "pie2023", title: "2023 Category Mix (Total LBS)" },
  { file: pluginAssetsUrl + "2024_log.csv", canvasId: "pie2024", title: "2024 Category Mix (Total LBS)" },
  { file: pluginAssetsUrl + "2025_log.csv", canvasId: "pie2025", title: "2025 Category Mix (Total LBS)" },
];
...
async function addCountyBoundaries(map) {
  const res = await fetch(pluginAssetsUrl + 'target_counties.geojson');               // Fetch the GeoJSON file for target counties
  if (!res.ok) throw new Error('Failed to load target_counties.geojson' + pluginAssetsUrl);
...
async function addTractLayer(map, geojsonPath = "tracts_acs_2024_elk_mar_sj.geojson") {
  const res = await fetch(pluginAssetsUrl + geojsonPath);
...
async function addBusRoutesLayer(map) {
  const res = await fetch(pluginAssetsUrl + "transpo_routes.geojson"); // or "data/transpo_routes.geojson"
